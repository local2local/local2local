...
elif [[ "${{ github.ref_name }}" == "staging" ]]; thenPROJECT_ID="local2local-staging"elsePROJECT_ID="local2local-dev"fi# Targeted deploy to force the CLI to re-evaluate the codebasefirebase deploy --only functions --project $PROJECT_ID --non-interactive --forceL2LAAF_BLOCK_ENDL2LAAF_BLOCK_START(typescript:Index:functions/src/index.ts)/**L2LAAF MAIN ENTRY POINT*/export * from "./logic/infrastructure";export * from "./logic/compliance";export * from "./logic/finance";export * from "./logic/orchestration";export * from "./logic/fulfillment";export * from "./logic/dispatch";export * from "./logic/ombudsman";export * from "./logic/analytics";export * from "./logic/treasury";export * from "./logic/evolution";export * from "./utilities/listSubcollections";export * from "./utilities/deleteSubcollection";L2LAAF_BLOCK_ENDL2LAAF_BLOCK_START(typescript:Evolution:functions/src/logic/evolution.ts)import { onDocumentUpdated } from "firebase-functions/v2/firestore";import { onRequest } from "firebase-functions/v2/https";import * as admin from "firebase-admin";const appIdStatic = "local2local-kaskflow";/**evolutionOnProposalWorkerV2Matches standard project naming convention.*/export const evolutionOnProposalWorkerV2 = onDocumentUpdated("artifacts/local2local-kaskflow/public/data/logic_proposals/{proposalId}",async (event) => {console.log("[EVOLUTION-V2] Wakeup for: " + event.params.proposalId);const newData = event.data?.after.data();if (!newData) return;const status = (newData.status || "").toUpperCase();const isCommitPending = newData.commit_pending === true;if (status === "APPROVED" && isCommitPending) {const db = admin.firestore();const hbrTarget = newData.hbrId || "UNKNOWN_HBR";try {const batch = db.batch();const lessonRef = db.collection("artifacts").doc(appIdStatic).collection("public").doc("data").collection("lessons_learned").doc();   batch.set(lessonRef, {
