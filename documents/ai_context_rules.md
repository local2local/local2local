...
## Assisted Method: Patcher v8.2 block format

**Context:** Any logic_payload generated by Gemini for use with `scripts/patcher.js`

**Rule:** Patcher v8.2 requires all blocks in a single-line inline format. Never use the deprecated multiline comment format.

**Correct format (v8.2) — no exceptions:**
```
L2LAAF_BLOCK_START(FILE:Human Readable Title:path/to/file.ts)
...full file content...
L2LAAF_BLOCK_END
```
...
```
L2LAAF_BLOCK_START(COMMIT_MSG:Commit Message:COMMIT_MSG)
[ASSISTED] FEAT(scope): Description [BUMP: MINOR]
L2LAAF_BLOCK_END
```

**Parameters:** `L2LAAF_BLOCK_START(type:title:filePath)` — all three on one line, colon-separated, no spaces around colons.

- `type` — `FILE` for source files, `COMMIT_MSG` for the commit message
- `title` — human-readable description of the file
- `filePath` — full relative path from repo root (e.g. `functions/src/logic/resolveHBR.ts`), or `COMMIT_MSG` for commit messages

**Deprecated format — never use:**
```
/* L2LAAF_BLOCK_START
...
This old multiline comment-wrapped format was used before patcher v8.2 and will cause patcher.js to silently skip the block.
